...
import { Loader2, Table2, Check, AlertCircle } from 'lucide-react';
import DropZone from '@/components/DropZone';
import styles from '@/app/merge/page.module.css';
import Link from 'next/link';

export default function PdfToExcelFeature() {
    ...
    return (
        <div className="container" style={{ paddingBottom: '4rem' }}>
            <header className={styles.header}>
                ...
                <h1 className={styles.title}>PDF to Excel</h1>
                <p className={styles.subtitle}>Convert PDF tables to editable Excel spreadsheets.</p>
            </header>
            ...
            <div className={styles.workspace}>
                {!file ? (
                    ...
                    <div className={styles.fileList} style={{ maxWidth: '600px', margin: '0 auto' }}>
                        ...
                        >
                            {isProcessing ? <Loader2 className="animate-spin" /> : 'Convert to Excel'}
                        </button>
                    </div>
                )}
            </div>
